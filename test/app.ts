...
$(function ()
{
    ...
    let form22 = document.getElementById('Search');
    form22.addEventListener("click", Show1, true);
   
    function Show1(e) {

        var inputValue = (<HTMLInputElement>document.getElementById('titler')).value;
        var list_Type = (<HTMLInputElement>document.getElementById('list_Type')).value;
        ...
        $.getJSON(url, function (data) {
            ...
            let x: number = data.totalResults;
            if (x == undefined) {
            ...
            let y: number = x / 10;
            let tr_btn = document.getElementById('tr_btn');
            for (let i = 0; i < y; i++)
            {
                let id = i + 1;
            ...
            for (let i = 0; i < 10; i++) {
                ...
                let text1 = document.createTextNode(data.Search[i].Type);
                span1.appendChild(text1);
                td2.appendChild(span1);
                tr2.appendChild(td2);

                span2.setAttribute('id', 'Title' + i);
                let text2 = document.createTextNode(data.Search[i].Title);
                span2.appendChild(text2);
                td3.appendChild(span2);
                tr3.appendChild(td3);

                span3.setAttribute('id', 'Year' + i);
                let text3 = document.createTextNode(data.Search[i].Year);
                ...
                if (i < 5) { div1.appendChild(div); }
                else { div2.appendChild(div); }
            }
        });
        ...
        function Show(e) {
            ...
            const url = `http://www.omdbapi.com/?${params}`;
            $.getJSON(url, function (data) {
                let div3 = document.getElementById('div33');
